...
thumbnail: assets/img/data_science/LR.jpg
toc:
  sidebar: left
---
...
We want to find a function $$f(x)$$ that predicts $$\mu(x)$$.
...
&= \sum_{i=1}^N[y_i(\ln\mu_i-\ln(1-\mu_i))+\ln(1-\mu_i)]\\
&= \sum_{i=1}^N[y_i\ln\frac{\mu_i}{1-\mu_i}+\ln(1-\mu_i)]\\
&= \sum_{i=1}^N[y_i\ln\exp(w^Tx_i)+\ln(\frac{1}{1+\exp(w^Tx_i)})]\\
&= \sum_{i=1}^N[y_iw^Tx_i-\ln(1+\exp(w^Tx_i))]\\
\end{aligned}
$$
