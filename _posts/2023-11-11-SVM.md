...
The larger the value of $$C$$, the more the model will penalise the slack variables, and the larger $$\lVert w \rVert$$ will be, which means the margin ($$\frac{2}{\lVert w \rVert}$$) will be smaller.
...
$$
L(w,b,\alpha) = \frac{1}{2}\lVert w \rVert^2 - \sum_{i=1}^n\alpha_i(y_i(w^Tx_i+b)-1)\; \alpha_i \geq 0
$$
...
where $$\lessdot$$ ,$$\gtrdot$$ is the inner product $$x_i$$ is the $$i$$th support vector, size of which is $$d \times 1$$, and $$\alpha_i$$ is the Lagrange multiplier of $$x_i$$.
